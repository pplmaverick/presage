          ...
          <div className="hidden md:flex gap-6 items-center">
            ...
            {showAdmin && (
              <NavLink
                to={ADMIN_ITEM.path}
                className={({ isActive }) =>
                  `font-display text-base tracking-wider transition-colors duration-200 pb-1 ${
                    isActive
                      ? 'text-[#ff9f45] border-b-2 border-[#ff9f45]'
                      : 'text-[#ff9f45]/70 hover:text-[#ff9f45]'
                  }`
                }
              >
                {ADMIN_ITEM.label}
        ...
        <div className="flex items-center gap-4">
          {isConnected && address && (
            <div className="hidden sm:flex flex-col items-end mr-1">
              ...
              <span className="text-[10px] uppercase tracking-widest text-tertiary">{activeChain.name}</span>
            </div>
          )}
          <WalletButton />
        </div>
          ...
          <div className="flex items-center gap-3 p-3 rounded-lg bg-[rgba(255,255,255,0.04)]">
            ...
            <div>
              ...
              <div className="flex items-center gap-1 mt-0.5">
                ...
                <p className="text-[10px] text-[rgba(255,255,255,0.5)]">{activeChain.name}</p>
              </div>
            </div>
          </div>
        </div>
          ...
          {showAdmin && (
            <NavLink
              ...
              className={({ isActive }) =>
                `flex items-center gap-3 px-4 py-3 rounded-lg transition-all duration-200 ${
                  isActive
                    ? 'bg-[#ff9f45]/15 text-[#ff9f45]'
                    : 'text-[#ff9f45]/70 hover:bg-[#ff9f45]/10 hover:text-[#ff9f45]'
                }`
              }
            >
              <span className="material-symbols-outlined text-xl">{ADMIN_ITEM.icon}</span>
              <span className="font-sans text-sm tracking-wider">{ADMIN_ITEM.label}</span>
            </NavLink>
          )}
        </nav>
        ...
        {showAdmin && (
          <NavLink
            ...
            className={({ isActive }) =>
              `flex flex-col items-center justify-center gap-0.5 transition-all ${
                isActive
                  ? 'text-[#ff9f45] drop-shadow-[0_0_8px_rgba(255,159,69,0.6)]'
                  : 'text-[#ff9f45]/60 hover:opacity-100'
              }`
            }
          >
            <span className="material-symbols-outlined text-xl">{ADMIN_ITEM.icon}</span>
            <span className="text-[10px] font-mono uppercase tracking-wider">{ADMIN_ITEM.label}</span>
          </NavLink>
        )}
      </nav>
    </div>
