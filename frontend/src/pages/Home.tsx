...
export default function Home() {
  ...
  const amountUnits = betAmount && !isNaN(parseFloat(betAmount))
      ...
      <div className="bg-slate-900 rounded-2xl p-6 border border-slate-800">
        <div className="flex items-start justify-between mb-4">
          <div>
            ...
            <p className="text-slate-400 text-sm mt-1">
              預測日期：{market ? new Date(Number(market.targetDate) * 1000).toLocaleDateString('zh-TW') : '—'}
            </p>
          </div>
        ...
        <div className="grid grid-cols-2 gap-4">
          <div className="bg-slate-800 rounded-xl p-3">
            ...
            <p className="text-white font-bold text-lg">{formatUsdc(totalPool)} USDC</p>
          </div>
          <div className="bg-slate-800 rounded-xl p-3">
            <p className="text-slate-400 text-xs mb-1">鎖盤時間</p>
            <p className="text-white font-bold text-sm">
              {market ? new Date(Number(market.lockTime) * 1000).toLocaleString('zh-TW', { timeZone: 'Asia/Taipei' }) : '—'}
            </p>
          </div>
        </div>
      ...
      <div>
        ...
        <div className="space-y-2">
          {Array.from({ length: BUCKET_COUNT }, (_, i) => {
            ...
            const isWinning = status === 2 && market?.winningBucket === i
            ...
            return (
              <button
                ...
                } ${(!isOpen || !isConnected) ? 'opacity-70 cursor-default' : 'cursor-pointer'}`}
              >
                <div className="flex items-center justify-between mb-2">
                  <div className="flex items-center gap-2">
                    ...
                    {isWinning && <span className="text-xs bg-yellow-500/20 text-yellow-400 px-2 py-0.5 rounded-full">🏆 得獎</span>}
                  </div>
                  <span className="text-slate-300 text-sm font-mono">{formatUsdc(total)} USDC</span>
                </div>
                <div className="h-1.5 bg-slate-800 rounded-full overflow-hidden">
                  <div
                    className={`h-full rounded-full transition-all ${isWinning ? 'bg-yellow-500' : 'bg-blue-500'}`}
                    style={{ width: `${poolPct}%` }}
                  />
                </div>
              </button>
      ...
      {isConnected ? (
        <div className="bg-slate-900 rounded-2xl p-6 border border-slate-800">
          <h3 className="font-semibold text-white mb-4">
            {isOpen ? '下注' : status === 1 ? '市場已鎖盤，不接受下注' : '市場已結算'}
          </h3>
              ...
              <div>
                <label className="text-sm text-slate-400 mb-1.5 block">
                  ...
                  <span className="text-white ml-1">
                    {selectedBucket !== null ? getBucketLabel(thresholds, selectedBucket) : '請選擇區間'}
                  </span>
                </label>
          ...
          {!isOpen && status === 2 && market?.noWinner && (
            <p className="text-slate-400 text-sm">無人猜中，請前往「我的下注」頁面領回押金。</p>
          )}
        </div>
      ) : (
